...
## Policy Engine
- Source lives under `src/policy/`. Public surface is the `PolicyViolationError` and `PolicyConfigurationError` classes, the `DEFAULT_POLICY_EXCLUSIONS` constant, and the `Policy*` / `SimulationResult` typedefs re-exported from `index.js`. Everything else under `src/policy/` is internal.
- The engine wraps account methods and protocol getters at `getAccount` time. When any policy applies to an account ("governed"), the proxy wraps **every callable** on the account and its prototype chain except the names in `DEFAULT_POLICY_EXCLUSIONS` (`src/policy/constants.js`) unioned with the consumer's `policyExclusions`. Coverage is deny-by-default: an unknown method is governed, not skipped. `docs/policy-exclusions-audit.md` is the source of truth for the default list — regenerate it (org endpoint, not `/users/tetherto/repos`, which hides private protocol packages) whenever a wallet or protocol package adds a public method. Combined with the default-deny evaluator (below) this closes sibling-method bypasses (e.g. a "cap transfer" policy cannot be sidestepped by `sendTransaction({ to: token, data: ERC-20-transfer-calldata })`, `approve` to an attacker, off-chain Permit via `signTypedData`, or ERC-7702 `delegate`).
...
- **Transaction-policy base (Phase 2 groundwork).** `src/policy/transaction-policy.js` exports `TransactionPolicy`, an abstract `match` / `evaluate` / `commit` / `rollback` contract for code-defined policies, and `src/policy/operation-record.js` defines the canonical `OperationRecord` those policies read. `src/policy/adapter-registry.js` (`AdapterRegistry`) turns `{ walletType, method, args }` into records via per-wallet extractor packs; `coerceAmount` normalises amounts to non-negative `bigint` or `undefined`; `PolicyAdapterError` is the fail-closed signal for a throwing or malformed extractor. These are exported and tested but **not yet wired into the proxy**: the engine still evaluates rule-shaped policies only. Concrete adapters and policies (EVM adapter, spending cap) live in `@tetherto/wdk-policies`. The name `Policy` remains the rule-shaped definition typedef; `TransactionPolicy` is the behavioural base.
- **`RulePolicy`** (`src/policy/rule-policy.js`) is a rule-shaped definition implemented as a `TransactionPolicy`. It reuses `validatePolicy`, `PolicyRegistry`, `buildContext` and `evaluate`, so within one policy the semantics are identical to the engine's; `tests/rule-policy.test.js` asserts verdict equivalence against the evaluator directly. Mapping: evaluator `ALLOW` → `allow`, a matching DENY rule → `deny`, rules that addressed the operation but whose conditions did not hold → `abstain`. Cross-policy composition (DENY-wins across policies, account-scope override of project DENYs) stays in the engine. `DEFAULT_CONDITION_TIMEOUT_MS` moved to `constants.js` so both the engine and `RulePolicy` read one value.
- Tests live in `tests/wdk-policy.test.js` (engine, exclusively through the public WDK API), `tests/policy-base.test.js` (the transaction-policy primitives) and `tests/rule-policy.test.js` (`RulePolicy` behaviour and evaluator equivalence).
